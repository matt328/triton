...
namespace tr::gfx::Geometry {
   ...
   std::unique_ptr<Mesh<Vertex, uint32_t>> MeshFactory::loadMeshFromGltf(
      ...
      if (tinygltf::TinyGLTF gltfContext;
         ...
         if (const auto node = model.nodes[scene.nodes[0]]; node.mesh > -1) {
            for (const auto mesh = model.meshes[node.mesh];
         ...
         return std::make_unique<Mesh<Vertex, uint32_t>>(allocator,
                                                         vertexBuffer,
                                                         indexBuffer,
                                                         transferContext);
      }
