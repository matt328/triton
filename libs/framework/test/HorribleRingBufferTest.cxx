...
TEST_CASE("HorribleStateBuffer handles wraparound", "[HorribleStateBuffer]") {
  ...
  buffer.pushState(state50, base + milliseconds(600));
  buffer.pushState(state50, base + milliseconds(700));
  buffer.pushState(state50, base + milliseconds(800));
  buffer.pushState(state60, base + milliseconds(900)); // Buffer is now full

  buffer.pushState(state70, base + milliseconds(1000));
  buffer.pushState(state80, base + milliseconds(1100));

  SECTION("Interpolates between latest valid frames") {
    auto result = buffer.getStates(base + milliseconds(1050));
    REQUIRE(result.has_value());
