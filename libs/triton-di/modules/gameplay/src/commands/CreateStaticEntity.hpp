...
namespace tr::gp::cmd {
   class CreateStaticEntityCommand
       : public ICommand<entt::registry&, const std::shared_ptr<gfx::ResourceManager>&> {
    public:
      explicit CreateStaticEntityCommand(std::string_view newModelFilename,
                                         std::string_view newEntityName)
      ...
      void execute(entt::registry& registry,
         ...
         auto modelData = resourceManager->createModel(std::filesystem::path{modelFilename});
