   ...
   ActionSystem::ActionSystem(std::shared_ptr<IEventBus> newEventBus)
      ...
      eventBus->subscribe<KeyEvent>([&](const KeyEvent& event) {
         ...
         if (event.buttonState == ButtonState::Pressed) {
            ...
            eventBus->emit(
                Action{.actionType = actionType, .stateType = stateType, .value = false});
         }
      });
      ...
      eventBus->subscribe<MouseMoved>([&](const MouseMoved& event) {
         ...
         if (deltaX != 0) {
            if (const auto xit = mouseActionMap.find(MouseInput::MOVE_X);
         ...
         if (deltaY != 0) {
            if (const auto yit = mouseActionMap.find(MouseInput::MOVE_Y);
      ...
      std::visit(
          [&]<typename T>(T&& arg) {
             ...
             if constexpr (std::is_same_v<U, Key>) {
                ...
                mouseActionMap.insert_or_assign(arg,
                                                Action{.actionType = aType, .stateType = sType});
