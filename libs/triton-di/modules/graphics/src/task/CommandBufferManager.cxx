    ...
    for (uint32_t i = 0; i < framesInFlight; ++i) {
      {
        ...
        commandPools[key] = std::make_unique<CommandBufferPool>(device, graphicsQueue->getFamily());
      }
      {
...
auto CommandBufferManager::getPrimaryCommandBuffer(const uint8_t frameIndex, const PoolId cmdType)
    -> CommandBufferPtr {
  ...
  if (const auto key = getKey(frameIndex, cmdType, queueFamily, true); commandPools.contains(key)) {
    return commandPools[key]->acquire();
  }
  throw std::out_of_range(std::format("Invalid frameIndex {} or queueFamily {} in getCommandBuffer",
                                      frameIndex,
                                      queueFamily));
}
