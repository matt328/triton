...
public:
  TritonModelData(const GeometryHandle geometryHandle,
      ...
      : geometryHandle(geometryHandle), imageHandle(imageHandle), skinData(std::move(skinData)) {
  }
...
struct hash<tr::GeometryHandle> {
  auto operator()(const tr::GeometryHandle& geometryHandle) const -> size_t {
    ...
    size_t hash2 =
        std::hash<int>{}(static_cast<int>(geometryHandle.topology)); // Assuming Topology is an enum
