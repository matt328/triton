...
AnimationSystem::AnimationSystem(std::shared_ptr<IAssetService> newAssetService,
                                 std::shared_ptr<EntityService> newEntityService)
    : assetService{std::move(newAssetService)}, entityService{std::move(newEntityService)} {
}
...
auto AnimationSystem::update() const -> void {
  ...
  const auto animationFn = [this]([[maybe_unused]] entt::entity entity, Animation& animationData) {
    ...
    const auto& animation = assetService->getAnimation(animationData.animationHandle);
    const auto& skeleton = assetService->getSkeleton(animationData.skeletonHandle);
