      ...
      void Draw(const char* title, bool* p_open = NULL) {
         ...
         if (ImGui::BeginChild("scrolling",
            ...
            if (Filter.IsActive()) {
               ...
               // and will fast-forward to skip non-visible lines. Here we instead demonstrate using
               // the clipper to only process lines that are within the visible area. If you have
               // tens of thousands of items and their processing cost is non-negligible, coarse
               // clipping them on your side is recommended. Using ImGuiListClipper requires
               ...
               // have random access into the data to display anymore, which is why we don't use the
               // clipper. Storing or skimming through the search result would make it possible (and
