...
RenderDevice::RenderDevice(const Instance& instance) {
  ...
  const auto rendererCreateInfo = RendererBaseCreateInfo{
      .device = *device,
...
void RenderDevice::waitIdle() const { device->waitIdle(); }
...
void RenderDevice::createLogicalDevice(const Instance& instance) {
  ...
  setObjectName(**device, *device.get(), vk::raii::Device::debugReportObjectType, "Primary Device");
...
void RenderDevice::createDescriptorPool() {
  const auto poolSize = std::array{
      vk::DescriptorPoolSize{
          .type = vk::DescriptorType::eUniformBuffer, .descriptorCount = FRAMES_IN_FLIGHT * 10},
      vk::DescriptorPoolSize{
...
void RenderDevice::createPerFrameData(const vk::raii::DescriptorSetLayout& descriptorSetLayout) {
  ...
  frameData[0]->getObjectMatricesBuffer().updateBufferValue(
      &objectMatrices, sizeof(ObjectMatrices));
}
...
vk::raii::RenderPass RenderDevice::defaultRenderPass() const {
  ...
  constexpr auto colorAttachmentRef = vk::AttachmentReference{
      .attachment = 0,  // index into the color attachments array
...
vk::Format RenderDevice::findSupportedFormat(
...
void RenderDevice::recordCommandBuffer(
    const vk::raii::CommandBuffer& cmd, const unsigned imageIndex) const {
...
vk::Extent2D RenderDevice::chooseSwapExtent(
    const vk::SurfaceCapabilitiesKHR& capabilities, const Instance& instance) const {
...
bool RenderDevice::isDeviceSuitable(
    const vk::raii::PhysicalDevice& possibleDevice, const Instance& instance) {
...
bool RenderDevice::checkDeviceExtensionSupport(
  ...
  std::set<std::string> requiredExtensions(
      desiredDeviceExtensions.begin(), desiredDeviceExtensions.end());
